...
│  │  ├── bbs_spider.py        # BBSSpider (论坛爬虫)               │   │
│  │  │   └── DiscuzSpider, PhpBBSpider, VBulletinSpider          │   │
│  │  ├── dynamic_news_spider.py # DynamicNewsCrawler (动态新闻)   │   │
│  │  └── spider_factory.py    # SpiderFactory (工厂模式)          │   │
│  └──────────────────────────────────────────────────────────────────┘   │
│  ┌──────────────────────────────────────────────────────────────────┐   │
│  │  检测器 (detector/) 🆕 v2.3                                      │   │
│  │  └── selector_detector.py  # SelectorDetector (选择器检测器)    │   │
│  │      # 用于自动检测论坛类型和选择器，辅助配置生成              │   │
│  └──────────────────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────────────────┘
                                    ↓
┌─────────────────────────────────────────────────────────────────────────┐
│                          解析层 (Parser Layer) 🆕 v2.3                  │
│  ┌──────────────────────────────────────────────────────────────────┐   │
│  │  parsers/                                                        │   │
│  │  ├── base.py            # BaseParser (解析器基类)              │   │
│  │  ├── bbs_parser.py      # BBSParser (论坛解析器)               │   │
│  │  └── dynamic_parser.py  # DynamicPageParser (动态页面解析器)   │   │
│  └──────────────────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────────────────┘
                                    ↓
┌─────────────────────────────────────────────────────────────────────────┐
...
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐                 │
│  │  下载器      │  │  存储器      │  │  去重器      │                 │
│  │ ImageDownload│  │  Storage     │  │ ImageDedup  │                 │
│  │ core/download│  │ core/storage │  │ core/dedup   │                 │
│  └──────────────┘  └──────────────┘  └──────────────┘                 │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐                 │
│  │  检查点管理  │  │  异步队列    │  │              │                 │
│  │ CheckpointMgr│  │ CrawlQueue   │  │              │                 │
│  │ core/checkpt │  │ core/queue   │  │              │                 │
│  └──────────────┘  └──────────────┘  └──────────────┘                 │
└─────────────────────────────────────────────────────────────────────────┘
                                    ↓
┌─────────────────────────────────────────────────────────────────────────┐
...
#### 分层架构（v2.3）
- **应用层**: 用户交互界面（CLI入口、配置管理、自动化脚本）
- **业务层**: 业务逻辑（爬虫策略spiders/、检测工具detector/）
- **解析层**: HTML解析逻辑（parsers/目录，独立于爬虫）
- **核心层**: 核心功能（下载、存储、去重、检查点、异步队列）
- **基础设施层**: 底层服务（HTTP客户端、数据库、文件系统）
...
- **Storage**：唯一持久化层。结果（threads / images / articles）+ 进度（checkpoints 表）。**「已爬」权威**仅在此：BBS 用 `thread_exists(thread_id)`，动态新闻用 `article_exists(article_id)`。
- **Checkpoint**：仅作**进度与状态**（薄封装，读写 Storage 的 checkpoints 表）。负责 current_page、last_thread_id、status（running/completed/error）、可选 stats；seen_article_ids 等仅作本轮+断点恢复的**缓存**，非权威。使用前需 `storage.connect()`。
- **CrawlQueue**：单次运行内的内存任务队列，不负责任务持久化。

详见 `docs/designs/2026-02-07-storage-checkpoint-queue-positioning.md`。
...
**文件**: `parsers/bbs_parser.py` (v2.3 文件结构重构)
...
**文件**: `detector/selector_detector.py`
...
**文件**: `spiders/bbs_spider.py` (v2.3 文件结构重构)
...
**文件**: `spiders/spider_factory.py` (v2.3 文件结构重构)

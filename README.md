...
当使用 `--download-images` 且首页可用 **Ajax** 时，采用 **三阶段流水线并行**（性能最大化）：

1. **阶段 1：列表页**  
   逐页请求列表（如 `?page=1,2,...`），解析出文章链接与标题，**每解析出新文章即推入队列**，供阶段 2 消费。

2. **阶段 2：文章详情**  
   多个 worker 从队列取文章 URL，并发拉取详情页，解析正文和图片链接，**每拿到图片 URL 即推入图片队列**，供阶段 3 消费。

3. **阶段 3：下载图片**  
   多个 worker 从图片队列取任务，并发下载到 `downloads/域名/`。某篇文章的**最后一图**下载完成后，才将该文章标记为已爬（`save_article(..., images_downloaded=1)`）。

因此会**边翻列表页、边拉详情、边下图**，无需等所有列表页爬完才开始下载。  
若首页无文章（需 Selenium），则自动回退为**三阶段串行**（先全部列表 → 再全部详情 → 再全部下图）。
